...
import { COACH_STAGE_DURATION_MS, COACH_STAGES, CoachRevealStage } from "@/components/cards/CoachRevealStage";
import ErrorScreen from "@/components/common/ErrorScreen";
import LoadingScreen from "@/components/common/LoadingScreen";
import { IconCoin } from "@/components/icons";
import { openClubCoachPack } from "@/api/clubCoachPacks";
import { ApiRequestError, staticUrl } from "@/lib/api";
import { haptic, hapticNotify } from "@/lib/telegram";
import type { ClubCoachPackOpenResult } from "@/types";

export default function ClubCoachPackOpenPage() {
  ...
  if (idempotencyKeyRef.current === null) {
    idempotencyKeyRef.current = `club-coach-pack-${packId}-${crypto.randomUUID()}`;
  }

  const [requestState, setRequestState] = useState<
    { status: "pending" } | { status: "success"; data: ClubCoachPackOpenResult } | { status: "error"; message: string }
  >({ status: "pending" });
  ...
  useEffect(() => {
    ...
    openClubCoachPack(Number(packId), idempotencyKeyRef.current!)
  ...
  const advance = () => {
    ...
    if (stageIndex < COACH_STAGES.length - 1) setStageIndex((i) => i + 1);
  };
  ...
  useEffect(() => {
    if (phase !== "revealing" || stageIndex >= COACH_STAGES.length - 1) return;
    timerRef.current = setTimeout(advance, COACH_STAGE_DURATION_MS);
    return () => {
      if (timerRef.current) clearTimeout(timerRef.current);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const skipAll = () => {
    ...
    setStageIndex(COACH_STAGES.length - 1);
  };

  if (requestState.status === "pending") return <LoadingScreen />;
  if (requestState.status === "error") {
    return <ErrorScreen message={requestState.message} onRetry={() => navigate("/clubs/coach-packs")} />;
  }
      ...
      {phase === "revealing" && (
        <div className="flex flex-1 flex-col">
          <CoachRevealStage
            ...
            stage={COACH_STAGES[stageIndex]}
            index={cardIndex}
            total={result.cards.length}
            onTap={advance}
          />
          {stageIndex === COACH_STAGES.length - 1 && (
            <div className="safe-bottom px-6 pb-6 pt-2">
              ...
              >
                {cardIndex < result.cards.length - 1 ? "Следующий тренер" : "Готово"}
              </button>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {result.cards.map((oc) => (
              <div key={oc.card.id} className="flex flex-col items-center gap-1 rounded-xl bg-bg-surface p-2">
                ...
                />
                <span className="truncate text-[10px] font-semibold text-ink-chalk">{oc.card.coach.display_name}</span>
                {oc.is_new && <span className="text-[9px] font-bold text-accent-green">Новый!</span>}
              </div>
            ))}
          </div>
