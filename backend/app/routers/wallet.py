...
from app.schemas.stars import StarsCoinInvoiceCreate, StarsCoinRateOut, StarsInvoiceCreateOut, StarsInvoiceStatusOut
from app.services import stars_payment_service

router = APIRouter(prefix="/wallet", tags=["wallet"])


@router.get("/stars-coin-rate", response_model=StarsCoinRateOut)
async def get_stars_coin_rate(db: AsyncSession = Depends(get_db)):
    return await stars_payment_service.get_coin_rate(db)
...
async def create_coin_invoice(
    ...
    return await stars_payment_service.create_coin_invoice(db, user, payload.stars_amount)
