...
**DB constraint discovered during Postgres verification:** `coin_transactions` has carried a `CHECK (balance_after >= 0)` constraint (`ck_coin_tx_balance_non_negative`) since the very first migration (`0001_initial.py`) — a DB-level backstop behind `wallet_service.debit_coins`'s balance check. It isn't mirrored in the SQLAlchemy model (so SQLite tests never exercised it) and only surfaced when this feature's negative-balance debit was manually verified against real Postgres, exactly the kind of gap CLAUDE.md's "verify locking-dependent changes manually against real Postgres" guidance exists for. A second migration (`0055`) relaxes the constraint to `balance_after >= 0 OR type = 'premium_subscription_adjustment'`, keeping the non-negative invariant for every other transaction type and every other code path (`credit_coins`/`debit_coins` still refuse to go negative) while allowing exactly the one new path that's supposed to.
